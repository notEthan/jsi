...
module JSI
  ...
  class Base
    ...
    class << self
      ...
      # @return [String] a string representing the class, showing represented schema names or URIs
      def inspect
      ...
      def schema_classes_const_name
        if respond_to?(:jsi_class_schemas)
          ...
          if !schema_names.any?(&:nil?) && !schema_names.empty?
            schema_names.map { |n| 'X' + n.gsub(/[^\w]/, '_') }.join('')
          end
        end
      end
...
#              jsi_schema_dynamic_scope: []
  ...
  #          jsi_schema_dynamic_scope: []
    ...
    def jsi_validate
      jsi_schemas.map { |schema| schema.validate_instance(self) }.inject(JSI::SchemaValidation::FullResult.new, &:+)
    end
    ...
    def jsi_object_group_text
      ...
      if (is_a?(PathedArrayNode) || is_a?(PathedHashNode)) && ![Array, Hash].include?(jsi_instance.class)
        if jsi_instance.respond_to?(:jsi_object_group_text)
          instance_txt = jsi_instance.jsi_object_group_text
        else
