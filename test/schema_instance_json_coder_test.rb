...
describe Scorpio::SchemaInstanceJSONCoder do
  let(:schema_instance_class) { Scorpio.class_for_schema(properties: {foo: {}, bar: {}}) }
  let(:options) { {} }
  let(:schema_instance_json_coder) { Scorpio::SchemaInstanceJSONCoder.new(schema_instance_class, options) }
  describe 'json' do
    describe 'load' do
      it 'loads nil' do
